...
Only writes grabber.blend and the three grabber_*.glb assets.
"""
import ast
import bpy
import math
...
beam('Shoulder / crossbar', (-.56, 1.35, -.08), (.56, 1.35, -.08), .14, 'rib')
for side in [-1, 1]:
    ball('Shoulder / socket', (side * .57, 1.35, -.08), (.21, .21, .21), 'gold')
...
for side in [-1, 1]:
    p0 = Vector((side * .57, 1.35, -.08))
    p1 = Vector((side * 1.60, 1.55, .02))
    p2 = Vector((side * 1.65, .15, .45))
    p3 = Vector((side * 1.33, .65, .75))
    points = [(1-t)**3*p0 + 3*(1-t)**2*t*p1 + 3*(1-t)*t*t*p2 + t**3*p3 for t in [i/18 for i in range(19)]]
