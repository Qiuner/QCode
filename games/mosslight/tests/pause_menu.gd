...
func run() -> void:
	...
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:pause-action", "payload": {"action": "nature"}})])
	assert(not game.nature_motion)
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:pause-action", "payload": {"action": "resume"}})])
	assert(not game.game_paused and not game.pause_panel.visible)
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:pause-action", "payload": {"action": "developer"}})])
	assert(game.game_paused, "developer panel pauses the world")
	var previous_status: String = game.sanctuary_computer.task_status
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:pause-action", "payload": {"action": "preview-review"}})])
