...
func run() -> void:
	...
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:locale", "payload": {"locale": "en"}})])
	...
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:locale", "payload": {"locale": "invalid"}})])
	check(TranslationServer.get_locale() == "en", "unknown bridge locale is ignored")
	game._on_qcode_message([JSON.stringify({"source": "qcode-host", "version": 1, "type": "world:locale", "payload": {"locale": "zh"}})])
	...
	var onboarding: Dictionary = game.residents.qcode_talk(game.residents.residents[0], false)
	check(onboarding.text.contains("项目文件夹") and onboarding.text.contains("工作区"), "QCode resident explains workspace binding")
	var ready_dialogue: Dictionary = game.residents.qcode_talk(game.residents.residents[0], true)
	check(ready_dialogue.text.contains("居民面板"), "QCode resident hands off to functional panel")
	var all_clear := true
