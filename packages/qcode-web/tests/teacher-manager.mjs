...
export async function testTeacherManager(browser, url) {
  ...
  try {
    await page.route('**/world/**', route => route.fulfill({ contentType: 'text/html', body: `<body><script>
      window.addEventListener('message', event => { if(event.data === 'test:teacher') parent.postMessage({source:'qcode-world',version:1,type:'resident:selected',payload:{residentId:'teacher'}},'*') });
      setInterval(()=>{for(const type of ['world:ready','world:playable'])parent.postMessage({source:'qcode-world',version:1,type},'*')},300)
    </script>` }))
    await page.goto(url)
    ...
    await page.screenshot({ path: '.qcode-home/teacher-sidebar-desktop.png' })
    ...
    await page.screenshot({ path: '.qcode-home/teacher-sidebar-mobile.png' })
