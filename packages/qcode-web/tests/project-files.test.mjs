...
test('project file reader confines paths, previews files and exposes read-only Git changes', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'qcode-files-'))
  ...
  const get = (params = {}, headers = { 'x-qcode-files': '1' }) => fetch(`${base}/?${new URLSearchParams({ project: 'test', ...params })}`, { headers })
  const git = (...args) => execFileSync('git', args, { cwd: project, windowsHide: true })
  try {
    assert.equal((await get({}, {})).status, 403)
    assert.equal((await get({}, { 'x-qcode-files': '1', origin: 'http://evil.test' })).status, 403)
    assert.equal((await get({}, { 'x-agent-isles-files': '1' })).status, 200, 'legacy header remains compatible')
