...
test('tutorial persists, protects project scope and requires completed execution plus explicit experience', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'qcode-tutorial-'))
  ...
  const post = body => fetch(url, { method: 'POST', headers: { 'x-qcode-tutorial': '1' }, body: JSON.stringify(body) })
  let run
  ...
  try {
    ...
    assert.equal((await fetch(url, { headers: { 'x-qcode-tutorial': '1', origin: 'http://evil.test' } })).status, 403)
    await command('start')
    ...
    const restored = await (await fetch(url, { headers: { 'x-qcode-tutorial': '1' } })).json()
    assert.equal((await fetch(url, { headers: { 'x-agent-isles-tutorial': '1' } })).status, 200, 'legacy header remains compatible')
    ...
    await handler.close()
    assert.equal((await fetch(url, { headers: { 'x-qcode-tutorial': '1' } })).status, 503)
  } finally {
