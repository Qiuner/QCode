...
// The historical domain name is persisted by DSH; changing it would orphan existing tutorial progress.
...
export function createTutorialHandler(ctx: Context, runs: KvTable<string, TutorialRun>) {
  ...
  return {
    async handle(req: IncomingMessage, res: ServerResponse) {
      ...
      if (!isDesktopRequest(req) && (!['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress ?? '') || !/^(127\.0\.0\.1|localhost|\[::1\]):\d+$/.test(req.headers.host ?? '') || (req.headers['x-qcode-tutorial'] !== '1' && req.headers['x-agent-isles-tutorial'] !== '1') || (req.headers.origin && req.headers.origin !== `http://${req.headers.host}`))) { reply(403, { error: '请求来源无效。' }); return }
      try {
...
export async function apply(ctx: Context) {
  ...
  for (const path of ['/qcode/tutorial', '/agent-isles/tutorial']) {
    ctx.effect(() => ctx.webServer.register({ kind: 'exact', path, handler: handler.handle }), `tutorial: endpoint ${path}`)
  }
}
