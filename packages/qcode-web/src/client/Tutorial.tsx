...
function tutorialError(error: unknown, t?: QCodeTranslate): string {
  const message = error instanceof Error ? error.message : String(error)
  return t && /[\p{Script=Han}]/u.test(message) && t('language.menu') !== '界面语言' ? t('tutorial.actionFailed') : message
}

export function useTutorial(actions: TutorialActions | undefined, workspaceId?: string, t?: QCodeTranslate) {
...
export function TutorialPanel({ tutorial, actions, project, pick, bindProject, submit, move, modelSettings, leave, composerTarget, previewTarget, running = false, waiting = false, nativeSessionId, t }: {
  ...
  const draftSuffix = `${run.id}:${run.step}${run.submission ? `:answer:${run.submission.requestId}` : ''}`
  const draftKey = `qcode:tutorial-draft:${draftSuffix}`
  const legacyDraftKey = `agent-isles:tutorial-draft:${draftSuffix}`
  useEffect(() => {
    let restored = run.submission ? '' : run.draft
    try { restored = readMigratedStorage(localStorage, draftKey, [legacyDraftKey]) ?? restored } catch {}
