...
export function NativeSidebar({ sessionId, toggleSidebar, close, t }: { sessionId?: string; toggleSidebar(): void; close(): void; t: QCodeTranslate }) {
  ...
  useLayoutEffect(() => {
    ...
    const root = document.querySelector('[data-qcode-town]')
    ...
    const place = () => {
      ...
      style.textContent = `
        [data-qcode-town] [data-slot="sidebar"] button:is([aria-label="收起侧边栏"], [aria-label="Collapse sidebar"]) { display: none; }
        [data-qcode-town] [data-slot="sidebar"] > * {
