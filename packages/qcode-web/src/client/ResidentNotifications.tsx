...
import type { QCodeTranslate, QCodeLocaleKey } from './locales.js'
...
type NoticeMessageKey = Extract<QCodeLocaleKey, `notification.${string}`>
...
const STORAGE_KEY = 'qcode.notifications.v1'
const LEGACY_STORAGE_KEY = 'agent-isles.notifications.v1'
...
export function ResidentNotifications({ sessions, activeSession, read, open, onCount, t, hidden = false }: {
  ...
  read: QCodeWorldInjected['readRecentSession']; open(item: NotificationSession): void
  ...
  const [saved] = useState(() => {
    try {
      const value = JSON.parse(readMigratedStorage(localStorage, STORAGE_KEY, [LEGACY_STORAGE_KEY]) ?? '{}')
