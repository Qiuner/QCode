...
export function NativeChat({ sessionId, t }: { sessionId: string; t: QCodeTranslate }) {
  const seat = useRef<HTMLDivElement>(null)
  useLayoutEffect(() => {
    const element = seat.current
    if (!element) return
    const style = document.createElement('style')
    style.dataset.qcodeNativeChat = ''
    document.head.append(style)
    const place = () => {
      const box = element.getBoundingClientRect()
      style.textContent = `
        [data-qcode-town] :is([data-slot="conversation"], [data-slot="main.conversation"]) {
          display: block !important; position: fixed; z-index: 21;
          left: ${box.left}px; top: ${box.top}px; width: ${box.width}px; height: ${box.height}px;
          visibility: visible; pointer-events: auto; overflow: hidden;
        }
        [data-qcode-town] :is([data-slot="conversation"], [data-slot="main.conversation"]) > [data-phase] {
          --dsh-chat-content-width: 100%; --dsh-composer-side-clearance: 0px;
          height: 100%; min-height: 0;
          --dsw-alias-bg-base: #fcfdf8; background: #fcfdf8;
        }
        [data-qcode-town] [data-slot="conversation.session.header"],
        [data-qcode-town] [data-width-handle] { display: none !important; }
        ...
        [data-qcode-town] :is([aria-label="选择工作区"], [aria-label="Select workspace"]) { display: none !important; }
        [data-qcode-town]:not([data-details-collapsed]) [data-slot="details"] {
