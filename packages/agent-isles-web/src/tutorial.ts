...
export function createTutorialHandler(ctx: Context, runs: KvTable<string, TutorialRun>) {
  ...
  const runCommand = async (input: unknown) => {
    ...
    switch (cmd.action) {
      ...
      case 'idea':
        if (run.step !== 'idea' || !cmd.draft?.trim()) throw new TutorialError('先告诉芽芽想做什么。')
      ...
      case 'followup': {
        ...
        if (cmd.action === 'followup') {
          ...
          if (previous.sessionId !== cmd.sessionId) throw new TutorialError('请在原会话回答芽芽。', 400)
          ...
          if (index < 0 || start?.type !== 'turn/start' || !events.some(event => event.type === 'turn/end' && event.data.turn === start.data.turn)) throw new TutorialError('芽芽仍在执行或等待审批，请先处理当前请求。')
        }
      ...
      case 'check': {
        ...
        if (!events.some(event => event.type === 'tool/result' && event.data.turn === end.data.turn && !event.data.error && !event.data.message.content[0].isError)) throw new TutorialError('还没有可核对的工具执行记录，请让芽芽实际制作并验证。')
      ...
      case 'confirm':
        ...
        if ((await artifact(run)).hash !== run.evidence.hash) throw new TutorialError('作品已经变化，请回到芽芽重新核对，不能确认旧预览。')
