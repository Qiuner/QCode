...
import { RESIDENTS } from './resident-model.js'
...
interface Notice extends NotificationSession { key: string; text: string; requiresAction: boolean }
const STORAGE_KEY = 'agent-isles.notifications.v1'

export function ResidentNotifications({ sessions, activeSession, read, open, onCount, hidden = false }: {
  ...
  useEffect(() => {
    ...
    for (const session of sessions) {
      const name = RESIDENTS.find(r => r.id === session.resident)!.name.split(' · ')[0]
      if (session.pending) {
        if (pendingKeys.current.get(session.id) !== session.pending) {
          publish({ ...session, key: `pending:${session.pending}`, text: session.pending.startsWith('approval') ? `${name}需要你确认` : `${name}想问你一件事`, requiresAction: true }, initialized.current.has(session.id))
      ...
      void read(session.id, controller.signal).then(result => {
        ...
        if (decision.unread) {
          publish({ ...session, key: `end:${end.seq}`, text: end.failed ? `${name}遇到了问题` : end.worked ? `${name}做完这一轮了，去看看结果` : `${name}有新回复`, requiresAction: false }, decision.announce)
  ...
  return createPortal(<div className="town-shell town-notification-layer" data-conversation={activeSession ? '' : undefined}><section className="town-notifications" aria-label="居民通知">
    {visibleToast && <div className="town-notification-toast" role="status"><button onClick={() => { open(visibleToast); setToast(undefined) }}><img src={RESIDENT_PORTRAITS[visibleToast.resident]} alt="" /><span><small>{visibleToast.projectName}</small>{visibleToast.text}</span></button><button aria-label="收起通知" title="收起通知" onClick={() => setToast(undefined)}><X size={16} /></button></div>}
    <button className="town-notification-toggle" aria-label={`居民通知，${notices.length} 条未读或待处理`} aria-expanded={expanded} onClick={() => setExpanded(value => !value)} title="居民通知"><Bell size={19} />{notices.length > 0 && <span>{notices.length}</span>}</button>
    {expanded && <div className="town-notification-list"><header><strong>居民来信</strong><button aria-label="关闭居民通知" onClick={() => setExpanded(false)}><X size={16} /></button></header>
      {(storageFailed || readFailed) && <p role="alert">{storageFailed ? '通知暂时无法保存，刷新后可能丢失。' : '部分通知暂时无法读取。'}{readFailed && <button onClick={() => setRetry(value => value + 1)}>重试</button>}</p>}
      {!notices.length && <p>暂时没有新的来信</p>}
      {notices.map(item => <div className="town-notification-item" key={`${item.id}:${item.key}`}><button onClick={() => { open(item); setExpanded(false); setToast(undefined) }}><img src={RESIDENT_PORTRAITS[item.resident]} alt="" /><span><small>{item.projectName}</small>{item.text}</span></button>{!item.requiresAction && <button aria-label={`标记已读：${item.text}`} title="标为已读" onClick={() => setNotices(current => current.filter(n => n.id !== item.id))}><Check size={16} /></button>}</div>)}
    </div>}
