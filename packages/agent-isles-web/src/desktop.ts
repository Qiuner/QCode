...
export async function apply(ctx: Context & ModelTestServices) {
  ...
  const handlers = {
    'resident-state': createResidentStateHandler(resolve(home, 'agent-isles-state.json')),
    'project-files': createProjectFilesHandler(ctx),
    'model-test': createModelTestHandler(ctx),
    tutorial: tutorial.handle,
  }
  for (const [name, handler] of Object.entries(handlers)) {
    ctx.connection.fetch.register({ path: `/api/agent-isles/${name}`, methods: ['GET', 'POST'], requestBody: 'buffered', fetch: desktopJson(handler) })
  }
}
