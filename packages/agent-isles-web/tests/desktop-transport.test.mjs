...
test('desktop carrier restricts origin, preserves validation and persistence', async () => {
  const root = await mkdtemp(join(tmpdir(), 'isles-desktop-'))
  try {
    const handler = desktopJson(createResidentStateHandler(join(root, 'state.json')))
    const url = 'dsh-app://app/api/agent-isles/resident-state'
    assert.equal((await handler(new Request('https://evil.test/api/agent-isles/resident-state'))).status, 403)
...
test('desktop carrier cancels bounded handlers and removes trust after return', async () => {
  ...
  await handler(new Request('dsh-app://app/api/agent-isles/test', { signal: controller.signal }))
