# QCode desktop launcher (local preview)
...
| macOS Intel (x86_64) | Portable ZIP containing `QCode.app` | Implemented, not published |
| macOS Apple Silicon (arm64) | Portable ZIP containing `QCode.app` | Implemented, not published |

## Windows usage

Run `qcode-setup-x64.exe`, then launch QCode from the desktop or Start menu. The launcher waits for the service and page to become ready, then opens the current authenticated URL in your default browser. Launching it again, or selecting "Open island" from the notification-area menu, reuses the same service. The operating system selects the port, which listens only on the local loopback interface.

Closing the browser does not stop active work. Select "Exit" from the QCode notification-area menu to stop the service. The process tree runs in a Windows Job Object, so normal or forced launcher termination also cleans up its child processes.
...
User data is stored in `%LOCALAPPDATA%\QCode\data`; installed files are stored in `%LOCALAPPDATA%\Programs\QCode`. On first launch, QCode moves the old `%LOCALAPPDATA%\agent-isles` data directory only when the new directory does not exist. If both directories exist, startup stops and asks for manual resolution rather than overwriting either copy. `QCODE_DATA_HOME` overrides the data path; the old `AGENT_ISLES_DATA_HOME` remains readable for compatibility. Uninstallation preserves user data. The preview installer still requires uninstalling the old program before an upgrade. The portable ZIP can be extracted and run directly and does not register an uninstall entry.

## macOS usage

The macOS portable archives have not been published yet; for now, build one on a Mac with the matching architecture by following the instructions below. Once published, use `qcode-darwin-x64.zip` on an Intel Mac or `qcode-darwin-arm64.zip` on an Apple Silicon Mac. Extract the entire archive to a short path and open `QCode.app`. The launcher appears in the menu bar and opens your default browser after the service is ready. Opening the app again asks the running instance to reopen the island. Use "Quit QCode" from the menu-bar item to stop it.

User data is stored in `~/Library/Application Support/QCode/data`, with the same legacy migration and conflict rules as Windows. The preview is unsigned. If Gatekeeper blocks it, right-click and choose Open, or allow it in System Settings. There is currently no `.pkg` installer or automatic updater.
