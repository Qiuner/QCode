...
构建使用 Windows 自带 .NET Framework C# 编译器，将当前 Node 可执行文件和已安装依赖复制进包；依赖应先通过仓库锁文件安装。构建下载对应 Node 版本的许可证；发行运行不需要联网安装依赖。`dist/desktop-*/` 包含安装 EXE、便携 ZIP 和 SHA256 校验文件，均不提交 Git。开发者 home、凭证和项目不会复制到发行包。

验证脚本实际解压安装 EXE 到带空格的独立目录，检查没有指向源码的链接，再使用全新数据目录测试认证、首页、世界资源、退出清理和原生模块加载。安装向导、桌面快捷方式、卸载 UI 和完整模型工作流仍需发布前人工验收。
