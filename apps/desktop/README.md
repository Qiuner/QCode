...
| macOS Apple Silicon（arm64） | 便携 ZIP（`Agent Isles.app`） | 预览 |
...
Intel Mac 下载 `agent-isles-darwin-x64.zip`，M 芯片 Mac 下载 `agent-isles-darwin-arm64.zip`。完整解压到较短路径后双击 `Agent Isles.app`。启动器在菜单栏显示图标；就绪后打开默认浏览器。重复打开应用会通知已运行的实例重新打开小岛。退出请使用菜单栏「退出 agent-isles」。
