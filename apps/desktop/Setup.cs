...
internal static class Setup {
    [STAThread] static int Main(string[] args) {
        ...
        string target = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "QCode");
        bool extract = args.Length == 2 && args[0] == "--extract";
        if (extract) target = Path.GetFullPath(args[1]);
        if (!extract && MessageBox.Show("将为当前用户安装 QCode，无需管理员权限。\n\n安装位置：" + target, "安装 QCode", MessageBoxButtons.OKCancel) != DialogResult.OK) return 0;
        Form progress = null;
        try {
            ...
            Label status = null;
            if (!extract) {
                progress = new Form { Text = "正在安装 QCode", Width = 430, Height = 150, ControlBox = false, StartPosition = FormStartPosition.CenterScreen };
            ...
            if (extract) return 0;
            string executable = Path.Combine(target, "QCode.exe");
            ...
            foreach (string folder in new[] { Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), Environment.GetFolderPath(Environment.SpecialFolder.Programs) }) {
                dynamic shortcut = shell.CreateShortcut(Path.Combine(folder, "QCode.lnk")); shortcut.TargetPath = executable; shortcut.WorkingDirectory = target; shortcut.Save();
            }
            using (var key = Microsoft.Win32.Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Uninstall\QCode")) {
                key.SetValue("DisplayName", "QCode"); key.SetValue("DisplayVersion", "0.0.0-preview"); key.SetValue("InstallLocation", target); key.SetValue("DisplayIcon", executable);
            ...
            MessageBox.Show("安装完成。点击桌面上的 QCode 即可进入小岛。\n退出请使用通知区域中的 QCode 菜单。", "QCode");
            Process.Start(executable); return 0;
        } catch (Exception error) { File.WriteAllText(Path.Combine(Path.GetTempPath(), "qcode-setup-error.log"), error.ToString()); if (!extract) MessageBox.Show(error.Message, "安装未完成"); else Console.Error.WriteLine(error.Message); return 1; }
