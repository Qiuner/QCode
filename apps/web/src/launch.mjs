...
const overlay = path.resolve(workspaceRoot, 'packages', 'qcode-web', 'cordis.patch.yml')
const home = resolveQCodeHome(workspaceRoot, process.env.DSH_HOME)
...
const resolvedOverlay = path.join(home, 'qcode.patch.yml')
writeFileSync(resolvedOverlay, readFileSync(overlay, 'utf8').replace("'@qcode/web-plugin'", JSON.stringify(pathToFileURL(require.resolve('@qcode/web-plugin')).href)))
...
process.exitCode = await supervise({
  ...
  env: { ...process.env, DSH_PERMISSION_MODE: process.env.DSH_PERMISSION_MODE ?? 'danger-full-access', DSH_HOME: home, QCODE_DIST_INDEX: require.resolve('@deepseek-ai/dsh-web-frontend/dist/index.html') },
  onLine(line, channel) {
    ...
    if (url) {
      ...
      if (process.env.QCODE_DESKTOP === '1' || process.env.AGENT_ISLES_DESKTOP === '1') console.log(line)
      else console.log('QCode：小岛已就绪，可使用“打开小岛.cmd”进入。')
