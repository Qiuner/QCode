...
agent-isles 需要使用 DeepSeek Harness 的 Agent、Workspace、Session、工具和 Web 能力，同时必须避免上游源码、npm runtime 与本地依赖解析各自漂移。
...
- 不把 agent-isles 修改提交到上游 submodule。
...
日常开发不依赖上游源码目录的构建状态，发布使用可校验的固定 runtime。升级成本会更显式，但上游变化不会静默进入 agent-isles。
