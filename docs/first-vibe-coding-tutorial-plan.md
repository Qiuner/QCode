...
## 9. 本轮改动 · 入口恢复首批（2026-09-21，已实现待验收）
...
验证：`corepack yarn typecheck`、`corepack yarn build:web` 与 Web 单元测试 66 项通过；入口桌面 1280×900 与窄屏 390×844 截图走查通过。

未覆盖（本批如实保留）：真实模型端到端首课、刷新 / 服务重启 / 取消目录选择与新手完整走查（见第 7 节验收矩阵）；多类型课程契约待对齐后另批推进。
