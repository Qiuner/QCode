...
## 9. 本轮改动 · 入口恢复首批（2026-09-22，已实现待验收）

范围按 issue #13 确认的边界执行：恢复创作手册"跟着学"区与教程接线，本批不建通用课程引擎、公共契约不绑定 HTML/Coding。

- 入口：创作手册新增"跟着学"区（`journal.learn*` / `journal.handbookNote` 中英键），无未完成课程显示"开始学习"，有则逐项目显示"继续学习 · 项目名"；自由创作保持默认，教程只由用户主动开启，首次进入不自动弹课。
- 门禁下移：`enterLearning` 不再要求模型配置；idea / folder 阶段可直接推进，真正提交模型任务时沿用既有检查（提交路径 `prepareResidentModel` 与 `ModelConfigurationRequired` 处理保留）。
- 暂停与恢复：点"开始创作"进入自由创作时恢复暂停进行中教程；`enterLearning(run)` 恢复既有 续接 / resume / returned 链路；面板 `TutorialPanel` 的离开与找回步骤沿用原实现。
- 命名统一：教程后端续接提示（`tutorial.ts` "从工作记录…"两处）统一为"创作手册"。
- 苔伯继续只承担项目、历史与创作手册入口职责，不恢复教学职责；真实执行、会话与审批仍由 DSH 原生能力承载。
- 删除无依赖的 Playwright 孤儿脚本 `tests/handbook-browser.mjs`，仍有效断言迁移至 `tests/tutorial-entry.test.mjs`（SSR 级 + 接线契约级），映射表见对应 PR 描述。

验证：`corepack yarn typecheck`、`corepack yarn build:web` 与 Web 单元测试 66 项通过；入口桌面 1280×900 与窄屏 390×844 截图走查通过；基础页面刷新续接已实机验证（刷新后手册显示"继续学习 · 项目名"并可恢复课程）。

未覆盖（本批如实保留）：真实模型端到端首课、课程各步骤中的刷新、服务重启、取消目录选择与异常恢复、新手完整走查（见第 7 节验收矩阵）；多类型课程契约待对齐后另批推进。
