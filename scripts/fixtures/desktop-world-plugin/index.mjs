...
export const name = 'qcode-desktop-world-probe'
...
export async function apply(ctx) {
  ...
  if (brandRoot) for (const file of await readdir(brandRoot)) {
    ...
    ctx.connection.fetch.register({ path: '/api/qcode/brand/' + file, methods: ['GET', 'HEAD'], requestBody: 'buffered',
  ...
  ctx.connection.fetch.register({
    path: '/api/qcode/preview-workbench.js', methods: ['GET', 'HEAD'], requestBody: 'buffered',
    async fetch(request) {
  ...
  ctx.connection.fetch.register({
    path: '/api/qcode/preview', methods: ['GET', 'HEAD'], requestBody: 'buffered',
    async fetch(request) {
  ...
  for (const relative of manifest.files) {
    ...
    const route = '/api/qcode/world/' + relative.split('/').map(encodeURIComponent).join('/')
    ctx.connection.fetch.register({
